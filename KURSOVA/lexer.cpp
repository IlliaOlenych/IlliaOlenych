...
unsigned int GetTokens(FILE* F, Token TokenTable[], FILE* errFile)
{
	...
	while (1)
	{
		...
		case Letter:
		{
			...
			else if (!strcmp(buf, "Integer32_t"))	temp_type = Type;
			else if (!strcmp(buf, "Stop"))		temp_type = EndProgram;
			...
			else if ((buf[0] >= 'A' && buf[0] <= 'Z') && (strlen(buf) == 12))
			{				
				bool valid = true;
